...
import 'package:chatbot_app_project/chatBot_project/initial_test_module/profile_domain_updating_screen.dart';
...
class WelcomingTestScreen extends StatefulWidget {
  const WelcomingTestScreen({super.key});

  @override
  State<WelcomingTestScreen> createState() => _WelcomingTestScreenState();
}

class _WelcomingTestScreenState extends State<WelcomingTestScreen> {
  ...
  final List<Map<String, String>> _welcomingContent = [
    {
      'image': 'assets/lotties/education.json',
      'title': 'Importance of Education',
      'content': 'Education is the key to unlocking a brighter future.'
    },
    {
      'image': 'assets/lotties/carrer.json',
      'title': 'Building Your Career',
      'content': 'Your career is shaped by your choices and learning journey.'
    },
    {
      'image': 'assets/lotties/life.json',
      'title': 'Choosing the Right Domain',
      'content': 'Pick a field that aligns with your passion and skills.'
    },
  ];

  void _onNextPressed() {
    if (_currentIndex < _welcomingContent.length - 1) {
  ...
  Future<void> addTestQuestions(List<Map<String, dynamic>> testModules) async {
    FirebaseFirestore firestore = FirebaseFirestore.instance;

    for (var module in testModules) {
      String domain =
          module['domain'].toString().toLowerCase().replaceAll(' ', '-');
      String level = module['level'].toString().toLowerCase();

      QuerySnapshot existingDocs = await firestore
          .collection('Test-Questions')
          .where('domain', isEqualTo: module['domain'])
          .get();

      DocumentReference domainRef;

      if (existingDocs.docs.isNotEmpty) {
        domainRef = existingDocs.docs.first.reference;
      } else {
        domainRef = firestore.collection('Test-Questions').doc();
        await domainRef.set({'domain': module['domain']});
      }

      CollectionReference questionsRef = domainRef.collection('Questions');

      for (var q in module['questions']) {
              ...
              children: [
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Expanded(
                        child: Text(
                            "if you'r not intrested to get in to test you can skip....Enjoy our App"),
                      ),
                    ],
                  ),
                ),
                Expanded(
                  child: PageView.builder(
                    ...
                    itemCount: _welcomingContent.length,
                    itemBuilder: (context, index) {
                      final content = _welcomingContent[index];
                      return Padding(
